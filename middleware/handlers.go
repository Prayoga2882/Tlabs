...
func TestDeleteStock(t *testing.T) {
	...
	// Create a mock for the deleteStock function
	mock.ExpectExec("DELETE FROM stocks WHERE stockid=$1").
		WithArgs(1).
		WillReturnResult(sqlmock.NewResult(1, 1))

	// Execute the deleteStock function
	deletedRows := deleteStock(1)

	// Check if the deleteStock function returns the expected value
	if deletedRows != 1 {
		t.Errorf("Expected 1, but got %v", deletedRows)
	}

	// Check if the mock is executed as expected
